...
DEFINE_LOG_CATEGORY(LogGMCOnlineSubsystem)
  ...
  if (OnlineSubsystem)
  {
    ...
    if (SessionInterface.IsValid())
    {
      ...
      if (OnlineSubsystem->GetSubsystemName() == "NULL")
      {
        ...
        UE_LOG(LogGMCOnlineSubsystem, Warning, TEXT("Using online subsystem NULL, only LAN sessions will be available."))
      }
      else
      {
...
#if !UE_VERSION_OLDER_THAN(4, 27, 0) && UE_VERSION_OLDER_THAN(5, 0, 0)
      // Workaround for a problem with Steam in 4.27. This must be true in order to be able to create game sessions.
...
bool USteamGameInstance::HostGameSession(const FString& SessionName)
{
  ...
  const APlayerController* LocalPlayerController = GetFirstLocalPlayerController();
  if (LocalPlayerController->HasAuthority())
  {
    if (SessionInterface.IsValid())
    {
      ...
      // ServerTravel is called once the session was successfully created (@see OnCreateSessionComplete).
...
void USteamGameInstance::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type ResultType) const
{
  ...
  else
  {
    const auto Reason = [ResultType]() -> FString {
      switch (ResultType)
      {
        case EOnJoinSessionCompleteResult::Success: checkNoEntryGMC() return "";
        ...
        case EOnJoinSessionCompleteResult::UnknownError: return "an unknown error occured.";
